...
export default function buildCard(titulo, descricao, url, imagem) {
  //function para criar o esqueleto dos cards
...
async function listaVideo(res) {
  try {
    const listaAPI = await conectaAPI.listaVideos(); //traz a lista de videos
    listaAPI.forEach(
      (
        elemento //Para cada item da listaAPI ele fará algo
      ) =>
        lista.appendChild(
          //Adiciona á lista, como filho, o elemento HTML criado na função buildCard()
          buildCard(
            //cria o elemento e preenche as lacunas de valores determiandas na função buildCard()
            elemento.titulo,
            elemento.descricao,
            elemento.url,
            elemento.imagem
